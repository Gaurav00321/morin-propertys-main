        ...
        <div className="section-container">
          <div className="space-y-8">
            {services.map((service, i) => (
              <div key={service.title} className="card !rounded-2xl overflow-hidden">
                <div className={`grid grid-cols-1 lg:grid-cols-2 ${i % 2 === 1 ? 'lg:grid-flow-dense' : ''}`}>
                  ...
                  <div className={`h-64 lg:h-auto bg-gradient-to-br from-brand-primary/20 to-brand-accent/10 flex items-center justify-center ${i % 2 === 1 ? 'lg:col-start-1' : ''}`}>
                    <service.icon size={80} className="text-brand-primary/20" />
                  </div>
                </div>
              </div>
            ))}
